...
class ProfileView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if form_type == "profile_pic":
                # Profile thumbnail
                thumb_path = get_profile_thumb_path(
                    request, form.files["profile_pic"].name
                )
    ...
    def get_context(self, form=None, form_type=None):
        ...
        context = {
            "title": "Profileeeeeeeeee",
            "user": self.request.user,
            "profile": profile,
            "profile_form": form
            if form_type == "profile_pic"
            else ProfilePicForm(instance=profile),
            "description_form": form
            if form_type == "description"
            else DescriptionForm(instance=profile),
