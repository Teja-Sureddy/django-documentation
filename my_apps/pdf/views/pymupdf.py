...
def processing_pdf_view(request):
    ...
    if request.method == 'GET':
        ...
        annot_file_path = get_temp_file_path(f'Invoice_annotations.pdf')
        ...
        for page in doc:
            ...
            for index, image in enumerate(images):
                ...
                file_path = get_temp_file_path(f"Invoice_image_{page.number + index}.{img_data['ext']}")

                with open(file_path, "wb") as file:
                    file.write(img_data['image'])

        # encrypt
        encrypt_file_path = get_temp_file_path(f'Invoice_encrypted.pdf')
        doc.save(encrypt_file_path, user_pw='user', owner_pw='owner', encryption=fitz.PDF_ENCRYPT_AES_256)
        ...
        decrypt_file_path = get_temp_file_path(f'Invoice_decrypt.pdf')
        doc_encrypted.save(decrypt_file_path)

        doc.close()

        return JsonResponse({
            'metadata': metadata, 'text': text, 'Message': 'Check the Temp directory for files starting with `Invoice`.'
        })
